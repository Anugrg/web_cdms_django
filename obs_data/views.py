...
class InsertObs(View):
    ...
    def post(self, request, req_payload=None):

        data = dict()
        data['error'] = None 
        data['message'] = 'insert successfull'

        # if req_payload is None parse json
        if req_payload is None:
            ...
            try:
                ...
                try:
                    ...
                    data['error'] = 'invalid request'
                    data['message'] = 'unperseable json data'
                    return JsonResponse(data)
            except:
        ...
        if req_data==None or type(req_data)!=list or len(req_data)==0:
            data['error'] = 'invalid json format'
            data['message'] = 'data is not provided or formatted properly'
            return JsonResponse(data)

        # get station_id list for country
        self.stations = set(
                            station.objects.all() \
...
class StationMeta(View):
    ...
    def get(self, request):
        ...
        stn_info_fields = ['id', 'name', 'full_name',
                            'station_id', 'wmo_id',
